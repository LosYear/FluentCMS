	...
	class MenuItem extends Translationable
	{
		...
		public function afterSave(){
			if(!$this->isNewRecord && $this->root_id == -1){
				...
				$translations = MenuItem::model()->findAll($criteria);
			...
			if($this->root_id != -1){
				$criteria = new CDbCriteria();
				$criteria->condition = '`lang_id` = :lang AND `parent_id` = :parent';
				$criteria->params = array(':lang' => $this->lang_id, ':parent' => $this->root_id);
