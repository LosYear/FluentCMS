...
<?php if (Yii::app()->user->isAdmin() && ($model->isNewRecord || $model->status != 1)): ?>
    <div class="form-group" id="social-export">
        <label class="col-lg-2 control-label"><?= Yii::t('AuthorModule.admin', 'Export to social networks') ?></label>

        <div class="col-lg-4">
            <?= $form->checkBox($advModel, 'exportVK', array('class' => '')) ?>
            <?= $form->label($advModel, 'exportVK', array('class' => '')) ?><br/>

            <?= $form->checkBox($advModel, 'exportTwitter', array('class' => '')) ?>
            <?= $form->label($advModel, 'exportTwitter', array('class' => '')) ?><br/>

            <?= $form->checkBox($advModel, 'exportFacebook', array('class' => '')); ?>
            <?= $form->label($advModel, 'exportFacebook', array('class' => '')) ?>
        </div>
    </div>
<?php endif; ?>
