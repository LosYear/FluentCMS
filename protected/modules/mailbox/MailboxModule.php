...
class MailboxModule extends CWebModule
{
	...
	*/
	...
	public function getDate($time)
	{
		$timediff = time() - $time;
		if($timediff < 60 )
			$date = $timediff . ' ' . Yii::t('MailboxModule.main','sec ago');
		elseif($timediff < 3600 - 60) // within last hour
			$date = ceil($timediff / 60) . ' ' . Yii::t('MailboxModule.main','min ago');
		elseif($time > strtotime('today')) // today
			$date = Yii::app()->dateFormatter->formatDateTime($time, null, 'short');
		elseif($time > strtotime(date('Y-1-01'))) // within this year
			$date = Yii::app()->dateFormatter->formatDateTime($time, 'long', null);
		else
			$date = Yii::app()->dateFormatter->formatDateTime($time, 'long', null);// last year or more
		return $date;
	}
