...
class YumRegistrationController extends YumController
{
    ...
    {
        if (!Yii::app()->user->isGuest && $this->action->id != 'activation')
    ...
    public function actionRegistration()
    {
        ...
        if (isset($_POST['YumRegistrationForm'])) {
            ...
            if (!$form->hasErrors() && !$profile->hasErrors()) {
                $user = new YumUser;
                $user->register($form->username, $form->password, $profile->email, 0);
                $profile->user_id = $user->id;
                $profile->save();

                $this->sendRegistrationEmail($user);

                $identity = new YumUserIdentity($form->username, $form->password);
                $identity->authenticate();
                Yii::app()->user->login($identity, 0);

                Yum::setFlash(Yum::t('Thank you for your registration. Please check your email.'));
                $this->redirect(array('/author/profile/activate'));
            }
        }
    ...
    public function sendRegistrationEmail($user)
    {
        ...
        Yii::app()->user->setFlash('success', Yii::t('journal', 'Activation email has been send to %s', array('%s' => $activation_url)));
        return $sent;
    }
